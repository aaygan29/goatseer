...
| `decision_making_trajectory/` | OpenNeuro ds002739 (EEG-fMRI random-dot-motion decision task), public, EEG only | **Null on real data (ADR-021):** within-trial state trajectories did not beat the occupancy baseline for perceptual choice, 4/4 subjects, 0/4 clear their shuffle null (mean gain -0.032). Honest null, not a confound; see its README. | G1 (provenance), G4 (specificity ablation: occupancy baseline + per-subject shuffle null) |
