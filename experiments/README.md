...
| `geometry_preserving_discretization/` | PhysioNet EEG-BCI | Tangent-space discriminant discretization (ADR-017) RECOVERS the signal the prototype discretization discarded: tangent-occupancy 0.635 (+0.094) tracks the MDM ceiling 0.604. Signal is static (occupancy), not trajectory. `neurospine.discretize`. | G1, G3 (prototype/MDM baselines), G4 (supervised-pipeline null) |
